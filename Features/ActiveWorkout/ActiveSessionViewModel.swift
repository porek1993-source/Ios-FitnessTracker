...
final class ActiveSessionViewModel: ObservableObject {
    ...
    private func startElapsedTimer() {
        // Invaliduj existující timer (prevence duplicitních timerů)
        elapsedTimer?.invalidate()

        elapsedTimer = Timer.scheduledTimer(withTimeInterval: 1.0, repeats: true) { [weak self] _ in
            // ⚠️ [weak self] v Timer closure je POVINNÉ
            guard let self else { return }
            // ✅ @MainActor třída: Timer callback je na main thread (RunLoop.main)
            // Proto je přímý přístup k @Published properties bezpečný
    ...
    func startRestTimer(seconds: Int) {
        ...
        restTimer = Timer.scheduledTimer(withTimeInterval: 1.0, repeats: true) { [weak self] timer in
            guard let self else {
                timer.invalidate()  // ⚠️ Pokud self je nil, invaliduj timer okamžitě
                return
            }
